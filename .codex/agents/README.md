...
Before dispatching `coderX`, Main Agent must create the `Dispatch Payload: coderX Task` defined in `.codex/skills/orchestrateX/modules/02-bus-payload.md`. This payload is the handoff contract and must be specific enough for coderX to execute without inferring user intent, mode, scope, output format, or verification obligations from conversation context. Include Workflow Mode, Objective, Parent/Child paths, Acceptance Source, Allowed Scope, Required Skills, Verification, and Output.

Before dispatching `evaluatorX`, Main Agent must create the `Dispatch Payload: evaluatorX Review Task` defined in `.codex/skills/orchestrateX/modules/02-bus-payload.md`. This payload must be specific enough for evaluatorX to audit without inferring audit target, acceptance scope, or review focus from conversation context. Include Child Path, Changed Files, Acceptance Source, Review Focus, and Output.
