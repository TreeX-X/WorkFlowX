...
Main Agent outputs this payload as the primary input when dispatching Agent(evaluatorX). evaluatorX must read this payload before deciding what documents or source files to inspect.
...
> Deep knowledge entries are **not** included in the prompt. The subagent reads the exact entry names from Parent 8.2 and retrieves detailed facts on demand from the knowledge files. This keeps incremental context lightweight without external MCP dependency.
