...
Per `orchestrator-playbook/SKILL.md` Hybrid Tree Section Map. evaluatorX-specific: read **Child Section 9** to inherit prior evaluation results (needed by partial mode).
...
#### Step 2B: Prompt-Based Mode (No PRD) [DEPRECATED — xlocal no longer uses this path]

> **Deprecated**: As of the xlocal optimization, Mode B always generates a Hybrid Tree before evaluation. This mode is retained for backward compatibility with other workflows that may lack hybrid documents, but xlocal will never trigger it.
